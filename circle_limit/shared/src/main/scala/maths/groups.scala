 ...
 */
class Group(generators: List[MoebiusTransformation]) {

  def allWordsOfLength(wordLength: Int) = {
    /**
     * Returns the set of all Moebius transformations which can be represented
     * as a word of length wordLength, with each element of the word
     * representing either a generator or an inverse of a generator.
     *
  ...
  private val generatorsAsMatrices = 
    generators map (t => t.theTransformationMatrix)

  private val generatorsAsMatricesWithInverses =
    generatorsAsMatrices map (m => (m, m.inverse))

  private val generatorsAndInverses = 
...
object Group {
  def apply(generators: List[MoebiusTransformation]) = 
    new Group(generators)
}
